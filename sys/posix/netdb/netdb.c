    ...
    if (nodename != NULL) { /* node location specified, try to resolve */
        if (hints->ai_flags & AI_NUMERICHOST) { /* no DNS lookup, just parse for an address string */
            ...
            if (addr.type == AF_INET) {
                ...
                if (!inet_pton(AF_INET, nodename, &addr.ipv4)) {
                    return EAI_NONAME;
                }
            #else
                return EAI_NONAME;
                ...
                if (!inet_pton(AF_INET6, nodename, &addr.ipv6)) {
                    return EAI_NONAME;
                }
            #else
                return EAI_NONAME;
            ...
            int res = sock_dns_query(nodename, addr_buf, hints->ai_family);
            switch (res) {
                case -ETIMEDOUT:
                    /* fall-through */
                case -EAGAIN:
        ...
        /* TODO: Code below taken from lwIP code. Functions are not available,
         *       so omit this case for now.
        if (hints->ai_flags & AI_PASSIVE) {
            ip_addr_set_any_val(hints->ai_family == AF_INET6, addr);
        }
        else {
